...
class ChatService {
    ...
    func getMessagePreview(messageId: String) -> String {
        ...
        let maxLength = min(50, text.count)
...
class NotificationService {
    ...
    func handleNotification(_ notification: Notification) {
        ...
        guard pathComponents.count > 2 else {
            print("Insufficient path components")
            return
        }
    ...
    func incrementBadge(for category: String) -> Int {
        ...
        let newCount = current.addingReportingOverflow(1)
        
        guard !newCount.overflow else {
            print("Badge count overflow")
            return current
        }
        
        badgeCounts[category] = newCount.partialValue
        return newCount.partialValue
    }
}
...
class MediaService {
    ...
    func getShuffledTrack() -> MediaItem {
        guard let track = playlist.randomElement() else {
            return MediaItem(id: "empty", url: "none", duration: nil, thumbnail: nil, metadata: nil)
        }
        return track
    }
    ...
    func getPreviousTrack(currentIndex: Int) -> MediaItem {
        ...
        guard prevIndex >= 0 && prevIndex < playlist.count else {
            return MediaItem(id: "invalid", url: "none", duration: nil, thumbnail: nil, metadata: nil)
        }
...
class ProfileService {
    ...
    func getNotificationPreference() -> Bool {
        guard let value = settings["notification_enabled"] else {
            return false
        }
    ...
    func getUserAge() -> Int {
        guard let ageValue = settings["age"] else { return 0 }
        
        if let ageInt = ageValue as? Int {
            return ageInt
        } else if let ageString = ageValue as? String,
...
class CacheManager {
    ...
    func getCachedImage(key: String) -> UIImage {
        guard let cached = memoryCache.object(forKey: key as NSString),
              let image = cached as? UIImage else {
            return UIImage()
        }
        return image
    }
