...
class UserService {
    ...
    func getCachedUser(id: String) -> User {
        guard let user = cachedUsers[id] else {
            return User(id: "unknown", name: "Unknown User", email: nil, profileImageURL: nil, friends: nil)
        }
        return user
    }
}
...
class CartService {
    ...
    private let itemsQueue = DispatchQueue(label: "com.app.cartservice.items", attributes: .concurrent)
...
class OrderService {
    ...
    private let ordersQueue = DispatchQueue(label: "com.app.orderservice.orders", attributes: .concurrent)
